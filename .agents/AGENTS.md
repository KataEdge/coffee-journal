...
- **Supabase CLI is the source of truth**: schema changes go through `supabase/migrations/*.sql`, applied with the [Supabase CLI](https://supabase.com/docs/guides/local-development/cli/getting-started) (`supabase migration new <description>` to create the file, `supabase db push` to apply it to the linked project). Write the migration file *first*, then apply it — never the reverse.
- **No Undocumented Schema Changes**: if a change is ever applied ad hoc (Supabase SQL Editor, `psql`, dashboard — e.g. because the CLI isn't set up yet in a given environment), a matching `.sql` file MUST still be added to `supabase/migrations/` (named `YYYYMMDDHHMMSS_description.sql`) in the SAME session/PR. Update `.agents/skills/supabase-schema/SKILL.md` too if the change affects the documented table shape.
- **One-time human setup** (the agent cannot do this — it requires interactive login / project credentials): `supabase login`, then `supabase link --project-ref <ref>` from the repo root. Once linked, `supabase db push` applies pending migrations to the live project.
- **Why**: A prior scale-widening change (1-5 → 1-10 taste ratings) was applied to Swift code but never migrated on the live DB, and no migration file existed to catch the drift — it silently broke saves in production until debugged live. See `supabase/migrations/20260731001200_widen_tasting_notes_scale_to_10.sql` for the recovered record (originally landed ad hoc under `docs/migrations/`, later moved here once the CLI workflow was adopted).
